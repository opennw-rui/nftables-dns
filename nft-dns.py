    ...
    for section in config.sections():
        if section != 'GLOBAL' and config[section].getboolean('enable', fallback=False):
            for fqdn in config[section]["domains"].split(','):
                ...
                # 检查set是否存在，不存在则创建
                check_and_create_set(family, table, set_name, section)
                ...
                if not (args.dry_run or (config.has_option('GLOBAL', 'verbose') and config['GLOBAL'].getboolean('dry_run', fallback=False))):
                    if "type ipv4_addr" in res:
                        ...
                        logging.error(f"Type of the {set_name} set not defined to \"ipv4_addr\" or \"ipv6_addr\" into the nftables set. Only theses type are allowed.")
                        exit(1)
                else:
...
def check_and_create_set(family: str, table: str, set_name: str, section: str):
    """检查set是否存在，如果不存在则创建"""
    # 首先检查表是否存在，如果不存在则创建表
    table_check_cmd = f"nft list table {family} {table}"
    table_exists = run_command(table_check_cmd, exit_on_error=False)
    
    if table_exists is None:
        logging.info(f"Table {table} in family {family} does not exist, creating it")
        create_table_cmd = f"nft add table {family} {table}"
        run_command(create_table_cmd)
    
    # 检查set是否存在
    set_check_cmd = f"nft list set {family} {table} {set_name}"
    set_exists = run_command(set_check_cmd, exit_on_error=False)
    ...
    if set_exists is None:
        ...
        if set_type not in ['ipv4_addr', 'ipv6_addr']:
            logging.error(f"Invalid set_type '{set_type}' for section {section}. Must be 'ipv4_addr' or 'ipv6_addr'")
            exit(1)
...
def run_command(cmd: str, exit_on_error: bool = True) -> str:
    """运行命令，如果exit_on_error为False，则在命令失败时返回None而不是退出"""
    ...
    if not (args.dry_run or (config.has_option('GLOBAL', 'verbose') and config['GLOBAL'].getboolean('dry_run', fallback=False))):
        try:
            ...
            if exit_on_error:
                ...
                logging.debug(f"Command failed (non-fatal): {e.stderr}")
                return None
        ...
        return "dry-run-output"  # 返回模拟输出用于dry-run模式
